...
def buildTestContract():
    ...
    contract = w3i.eth.contract(abi=contract_io['abi'], bytecode=contract_io['bin'])

    # get tx hash from deployed contract
    tx_hash = contract.deploy(transaction={'from': w3i.eth.accounts[0], 'gas': 420000})
    ...
    abi = contract_io['abi']
    contract_instance = w3i.eth.contract(address=contract_address, abi=abi, ContractFactoryClass=ConciseContract)

    # getters & setters for web3.eth.contract object
    print('Contract value: {}'.format(contract_instance.greet()))
    contract_instance.setGreeting('d\'herroo', transact={'from': w3i.eth.accounts[0]})
